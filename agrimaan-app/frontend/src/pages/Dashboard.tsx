...
const Dashboard: React.FC = () => {
  ...
  const cropsReadyToList = crops.filter(crop => 
            ...
            <CardContent>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <Box>
                  ...
                  <Typography variant="h4" component="div">
                    {cropsLoading ? '...' : cropsReadyToList.length}
                  </Typography>
                </Box>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 2, height: '100%' }}>
            ...
            ) : cropsReadyToList.length === 0 ? (
              <Typography color="text.secondary">
                No harvested crops ready to list at the moment
              </Typography>
            ) : (
              <List dense>
                {cropsReadyToList.slice(0, 4).map((crop) => (
                  <ListItem 
                    key={crop._id} 
                    sx={{ px: 0, mb: 1 }}
                  >
